...
// const App = () => {
//  function handleLoadingState(props){
//     return props.state;
//   }
//   const [loading,setLoading]=useState("");
//  const [currTourList,setCurrTourList]=useState(tourList);
//  function afterDelete(item_id){
//   setCurrTourList(prevTours => prevTours.filter((item)=> item.id !== item_id));
//   if(currTourList.length === 0) {
//     return (
//       <div>
//         <h1>No tours left</h1>
//         <button onClick={() => setCurrTourList(tourList)}>Refresh</button>
//       </div>
//     )
//   }
// }
...
//       <main id="main">
//         {
//           currTourList.map((item)=>{
//            return <Tour id={item.id} name={item.name} info={item.info} image={item.image} price={item.price} afterDelete={afterDelete}/>
//           })
//         }
//       </main>
//     )
    ...
    <main id="main">
      {
        ...
        currTourList.map((item) => {
          return <Tour key={item.id} id={item.id} name={item.name} info={item.info} image={item.image} price={item.price} afterDelete={afterDelete}/>
        }) :
        <div>
          <h1>No tours left</h1>
          <button onClick={() => setCurrTourList(tourList)}>Refresh</button>
